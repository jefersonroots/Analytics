                    ...
                    where 
                      ...
                      $queryParte3 ="  --INFORMA VENDA A VENDA TODOS OS REGISTROS ORIGINADOS COMO PRE-VENDA
						...
						select
                            ...
                            convert(varchar,lp.DATA_DIGITACAO,114) as 'PEDIDO',
							convert(varchar,lp.DATA_PARA_TRANSFERENCIA,114) as 'CONFERIDO',
							...
							format(lp.DESCONTO, 'C', 'pt-br')  AS 'DESCONTO',
                        ...
                        from
                            ...
                            left join (SELECT TOP 1 CODIGO_FILIAL_ORIGEM,PEDIDO,TICKET,DATA_VENDA from LOJA_PEDIDO_VENDA where DATA_VENDA = @data group by CODIGO_FILIAL_ORIGEM,PEDIDO,TICKET,DATA_VENDA) lpv on lpv.CODIGO_FILIAL_ORIGEM = lp.CODIGO_FILIAL_ORIGEM and lpv.PEDIDO = lp.pedido
                        where
                        ...
                        select
                            ...
                            convert(varchar,lv.DATA_DIGITACAO,114) as 'PEDIDO',
							convert(varchar,lv.DATA_PARA_TRANSFERENCIA,114) as 'CONFERIDO',
							...
							format(lv.DESCONTO, 'C', 'pt-br')  AS 'DESCONTO',
                            LV.TERMINAL,
                 ...
                 }else 
                 {
                  ...
                  //  --EXECUTAR EM TODAS AS LOJAS, COM EXCEÇÃO DAS LOJAS 000303, 000304, 000305, 000306, 000307, 000308, 000309, 000310, 000312, 000313, 000314, 000315, 000316 E 000203.
                    ...
                    where 
                       ...
                       select
                           lp.PEDIDO as 'REGISTRO',
                           (select convert(varchar,max(lpo.DATA_INCLUSAO),114) from loja_pedido_produto lpo where lpo.codigo_filial_origem = lp.codigo_filial_origem and lpo.PEDIDO = lp.PEDIDO)  as 'PEDIDO',
						   convert(varchar,lp.DATA_PARA_TRANSFERENCIA,114) as 'CONFERIDO',
                           ISNULL(lp.identificacao_cliente,'NAO IDENTIFICADO') AS 'CLIENTE',
                           case when lp.tipo_pedido = 3 and lp.LX_TIPO_PRE_VENDA <> 2 and lp.cancelado = 0 and lp.DIGITACAO_ENCERRADA = 1 and lp.ENTREGUE = 0 then
                               'PEDIDO FECHADO'
						   ...
						   case when lpv.TICKET is not null then
                               (select format(lv.DESCONTO, 'C', 'pt-br') from loja_venda lv where lv.CODIGO_FILIAL = lp.CODIGO_FILIAL_ORIGEM and lv.TICKET = lpv.TICKET)
                               when lpv.TICKET is null and (lp.tipo_pedido = 3 and lp.LX_TIPO_PRE_VENDA <> 2 and lp.cancelado = 0 and lp.DIGITACAO_ENCERRADA = 1 and lp.ENTREGUE = 0) then
                               format(lp.DESCONTO, 'C', 'pt-br')
                           else
                               format(0.00, 'C', 'pt-br')
                           end as 'DESCONTO',
                           case when lpv.TICKET is not null then
				...
				<div  style='overflow: auto;'class='panel panel-primary'>
					...
					<nav class="navbar  style='overflow: auto;' navbar-light bg-light" ALIGN="center">
						...
						echo "
							...
							<th align='center'><div align='center'>  <h6><b>PEDIDO</h6></b> </th>
							<th align='center'><div align='center'>  <h6><b>CONFERIDO</h6></b> </th>
							...
							<th align='center'><div align='center'>  <h6><b>DESCONTO</h6></b> </th>
						...
						while (@$itemc = sqlsrv_fetch_array($ResultadoSQL2, SQLSRV_FETCH_ASSOC)) {
							...
							echo "
                                    ...
                                    <TD align='center' class='col-md-2'><font size=2>" . $itemc['PEDIDO'] . "</font></TD> 
									<TD align='center' class='col-md-2'><font size=2>" . $itemc['CONFERIDO'] . "</font></TD> 
									...
									<TD align='center' class='col-md-2'><font size=2>" . $itemc['DESCONTO'] . "</font></TD> 
