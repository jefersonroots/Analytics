...
from
	...
	SELECT
		...
		CAST(ROUND((SUM(A.VALOR_PAGO) /
			(select max(valor) from (SELECT a2.vendedor, sum(a2.valor_pago) as 'valor' FROM LOJA_VENDA A2 WHERE A2.DATA_HORA_CANCELAMENTO IS NULL AND A2.CODIGO_FILIAL = @filial and A2.DATA_VENDA between @dataini and @datafim group by A2.VENDEDOR) t)) * 100,2) AS NUMERIC(11,2)) as '%_RNK',
		CASE WHEN B.DESC_CARGO LIKE 'VENDED%' THEN
			CAST(ROUND((select SUM(PV.PREVISAO_VALOR) from LOJAS_PREVISAO_VENDAS PV WHERE PV.FILIAL = C.FILIAL AND PV.DATA_VENDA between @dataini and @datafim) /
